/*
KNXnet/IP CONNECTIONSTATE_REQUEST Frame body
...
const {
    KNXNETIP_SERVICE_TYPES : { CONNECTIONSTATE_REQUEST }
} = rrequire('../../constants');
const HPAI = require('../../core/HPAI');

class FrameBody__CONNECTIONSTATE_REQUEST extends BaseKNXStructure {
    constructor(data) {
        if (!(data instanceof Buffer)) {
            if (!(data.control_endpoint instanceof HPAI)) data.control_endpoint = HPAI.from(data.control_endpoint);
        }
        super(data);
        this.service_type = CONNECTIONSTATE_REQUEST;
    }
...
module.exports = FrameBody__CONNECTIONSTATE_REQUEST;