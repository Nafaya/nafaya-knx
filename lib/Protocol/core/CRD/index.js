...
const CRDHeader = require('./header');
const {
    CONNECTION_TYPES
} = require('../../constants');
...
class CRD extends BaseKNXStructure {
    constructor(data) {
        if (!(data instanceof Buffer)) {
            if (!(data.header instanceof CRDHeader)) data.header = CRDHeader.from(data.header);
        }
        super(data);
    }
    update(data) {
    ...
    static fromBuffer(buffer) {
        const header = CRDHeader.from(buffer.slice(0, 2));
